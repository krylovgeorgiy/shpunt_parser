...
class LocatorAvito:
    ...
    GEO = (By.CSS_SELECTOR, "[class='styles-module-noAccent-nZxz7']")
...
class AvitoParse:
    ...
    def __get_url(self):
        ...
        if "Доступ ограничен" in self.driver.get_title():
            time.sleep(10)
            raise Exception("IP Block -- reload")
    ...
    def __paginator(self):
        """Кнопка далее"""
        logger.info('Load success. Start checking.')
        ...
        while self.count > 0:
            ...
            if self.driver.find_elements(LocatorAvito.NEXT_BTN[1], by="css selector"):
                ...
                logger.info("Doesnt exist next page")
                break
    ...
    def __parse_page(self):
        ...
        for title in titles:
            ...
            geo = title.find_element(*LocatorAvito.GEO).text
            global GEO
            GEO = geo
            ads_id = title.get_attribute("data-item-id")
            description = title.find_element(*LocatorAvito.DESCRIPTIONS).text
            #ban_description = AvitoParse.__ban_description()
            ban_description = ['Наша компания — надежный российский поставщик шпунта', 'крупнейший продавец горячекатаных и холоднокатаных', 'горячекатаных', 'холоднокатаных', 'максимально доступные цены', 'ведущим поставщиком', 'Качество нашей продукции', 'Ответственно выполняем все свои', 'Ваши вопросы!', 'Не откладывайте','ИЗБРАННОЕ', 'выгоды', 'гибкие условия сотрудничества с нашими постоянными заказчиками']
            ...
            #if self.ban_description != ['']:
            #    if any(item.lower() not in description.lower() for item in self.ban_description) \
            #        and \
            #            self.min_price <= int(
            ...
            if self.keys_word != ['']:
                if any([item.lower() in (description.lower() + name.lower()) for item in self.keys_word]) and self.min_price <= int(price) <= self.max_price:
                        if all(item.lower() not in description.lower() for item in ban_description):
                            self.data.append(self.__parse_full_page(url, data))
    ...
    def __pretty_log(self, data):
        ...
        logger.success(
            ...
            f'#{GEO.split(",")[-1].replace(" ", "").replace(".", "").replace("-", "")}\n'
    ...
    def __ban_description() -> list:
        with open("ban_description.txt", "r") as file:
            ban_description = file.read().split("\n")
            #lines = ban_description.splitlines()
            #for line in lines:
            #    if line in ban_description:
            #        return True
        return ban_description

    def open_ban_description():
        with open("ban_description.txt", "r") as file:
            ban_description = file.read().split("\n")
        return ban_description
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            logger.info("Pause")
            time.sleep(int(10) * 60)
        except Exception as error:
            logger.error(error)
            logger.error('Error,  30 сек. ')
            time.sleep(30)
